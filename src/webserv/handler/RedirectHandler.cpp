...
RedirectHandler::RedirectHandler(const HttpRequest &request, HttpResponse &response) : AHandler(request, response) {}

void RedirectHandler::handle()
{
    if (request_.getUri().isRedirect())
    {
        Log::info("Redirecting request to: " + request_.getUri().getRedirect().second
                  + " with reason: " + Http::getStatusCodeReason(request_.getUri().getRedirect().first));
        ...
        std::string body = "<html><head><title>" + std::to_string(redirect.first) + " "
                           + Http::getStatusCodeReason(redirect.first)
                           + "</title></head>"
                             "<body><a href=\""
